...
const UiButton: FC<IProps> = ({
    ...
    let classes =
        'inline-block text-cyan-400 dark:text-cyan-300 hover:text-cyan-500 dark:hover:text-cyan-400';

    if (variant === 'text-btn') {
        classes =
            'px-4 py-2 text-cyan-400 dark:text-cyan-300 hover:text-cyan-500 dark:hover:text-cyan-400';
    }

    if (variant === 'text-attention') {
        classes = 'text-red-500 px-4 py-2 hover:text-red-600';
    }

    if (variant === 'solid') {
        classes =
            'text-zinc-800 px-4 py-2 after:z-10 before:absolute after:absolute before:inset-0 after:inset-0 before:rounded-md after:rounded-md before:duration-300 after:duration-300 before:ease-in-out after:ease-in-out before:transition-all after:transition-all before:bg-gradient-to-br after:bg-gradient-to-tl before:from-cyan-200 after:from-cyan-200 before:via-cyan-300 after:via-cyan-300 before:to-cyan-400 after:to-cyan-400 after:opacity-0 hover:after:opacity-100';
    }

    if (variant === 'outline') {
        classes =
            'text-zinc-800 px-4 py-2 dark:text-zinc-300 border-2 border-zinc-800 dark:border-zinc-300 rounded-md hover:text-cyan-500 dark:hover:text-cyan-300 hover:border-cyan-500 dark:hover:border-cyan-300';
    }
