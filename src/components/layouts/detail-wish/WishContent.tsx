...
const WishContent: FC<IProps> = ({ wish }) => {
    ...
    return (
        <div
            className={`${wish.images.length > 1 ? 'mt-[100px] tablet-md:mt-[120px] desktop-xs:mt-0' : ''} flex w-full flex-col gap-4 px-4 tablet-md:px-5 tablet-lg:px-8 desktop-xs:col-span-5 desktop-xs:gap-6`}
        >
            ...
            <div className="flex h-auto w-full flex-col gap-4 desktop-xs:max-h-[70svh] desktop-xs:gap-6 desktop-xs:overflow-y-auto desktop-xs:pr-2">
                {showRow && (
                    <div className="flex flex-col gap-4 desktop-xs:gap-5">
                        {myUser?.id === wish.userId && (
                            <p className="whitespace-nowrap text-zinc-600 dark:text-zinc-400">
                                {show}
                            </p>
                        )}
